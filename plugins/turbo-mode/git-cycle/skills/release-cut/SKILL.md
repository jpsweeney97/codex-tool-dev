...
- **Landing the cut can itself publish.** Where the versioned unit's runtime serves or auto-syncs from landed source (local-marketplace plugins on this machine: Claude serves landed source live, and the Codex app-server syncs a landed bump to its cache unprompted), landing the cut is going live — so land a cut only at publish intent, and when an open gate or proving sequence must hold publication, land the work without the cut and cut the release as a follow-up at grant time.
- **Name, but do not fire, the outward publish train.** Print the exact gated next steps for the user to authorize and run none of them. For a distributed plugin this is, in order: land the branch → republish the plugin to its runtime cache → update any distribution mirror → push. Publishing, tagging, pushing, republishing, and mirroring are outward acts that happen only on explicit authority — except the local runtime-cache republish that completes a landed cut where the previous bullet applies.
