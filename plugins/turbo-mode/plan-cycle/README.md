...
- `implement-issue` — consume one agent-ready tracker issue or triaged pull request per implementer: verify its blockers are closed, implement exactly its scope, prove each acceptance criterion by behavior, and hand off to the appropriate landing or pull-request lane. Asked for several ready issues where subagent tooling with isolated worktrees exists, its batch mode runs that contract once per issue in parallel and lands the branches locally; publication stays approval-gated.
...
- `spec-drift-reconcile` — when intent changed mid-arc, surface each stale artifact with two-sided evidence and blast radius, record the human's direction decision under `docs/reconciliations/` unless the repo names another location, then drive the recorded repair mode through the owning skills.

The forward chain is `to-prd` → `acceptance-map` → `implementation-planning`, then either → `execute-plan` for a plan run or → `to-issues` → `implement-issue` for tracker slices; each member routes to its neighbors by name and stops at the boundary. `triage` and `implement-issue` are the tracker side of the same arc. `plan-queue` is the repo-sweep variant that needs no tracker. `spec-drift-reconcile` is the reconciliation path the forward-only chain otherwise lacks.
...
The tracker skills (`to-prd`, `to-issues`, `triage`, `implement-issue`) expect the project's issue tracker and its triage-label vocabulary to be configured. A configured tracker must support creating items, reading items with comments, commenting, applying and removing role labels, closing, listing by role, and determining whether a blocker is closed. Ask the user to run `/setup-matt-pocock-skills` where that user-invoked skill is available; otherwise each tracker skill asks the smallest setup question it needs before a tracker-dependent read or mutation.

Companion skills are used only where available: `baseline`, `contract-change-propagation`, `tdd`, `keep-green`, `closeout-check`, `merge-branch`, `git-cycle`, `migration-campaign`, and `migration-safety`. Each skill body states its safe fallback or stop when a companion is unavailable. `update-config` is a Claude Code built-in rather than a cross-runtime skill; when it is unavailable, `plan-queue` hands the guard-allowlist edit to the user.
...
| `implement-issue` | Code changes and commits on a working branch; a proposed closing or status comment posted only on approval. Never closes the parent. In batch mode, also the local fast-forward landing of each implementer's branch into the integration branch; push, issue closure, and worktree cleanup only on approval. |
...
| `spec-drift-reconcile` | A direction record at `docs/reconciliations/YYYY-MM-DD-<topic>.md` unless the repo names another location; owners apply fixes where they exist, while ownerless forward-create-only artifacts use the recorded supersession default or an explicitly selected surgical edit. |
