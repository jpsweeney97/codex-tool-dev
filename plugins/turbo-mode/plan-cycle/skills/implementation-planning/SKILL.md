...
Close honestly. The reference class is one you drew, and a base rate is a prior, not a guarantee — this debiases the plan, it does not certify it complete. Name the class you compared against and where the comparison came from: the past plans, PRs, or changes you read, by name, or general knowledge only when the repo had no comparable record. Say which — a comparison drawn from memory is weaker than one drawn from the record, and the plan should not hide the difference. Do not stamp the plan comprehensive.
