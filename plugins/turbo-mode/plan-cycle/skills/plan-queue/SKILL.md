...
A branch-protection guard may refuse a write at the repository root while a protected branch is checked out: the queue is untracked scaffolding, but the guard only sees a write. Say so and hand the choice back — cut a working branch to hold the queue, or allowlist `PLAN-*.md` in the guard, which is `update-config`'s. Never relocate the queue to slip past it; the filenames at the root *are* the queue.
...
2. Cut a working branch, named to the repo's own convention — a guard that enforces one will reject a bare slug. Execute under `execute-plan`'s contract: its review gates, its status protocol, its stop conditions. That skill owns the execution, and if it stops, its finding is the answer.
...
4. Land locally under `merge-branch`'s contract: fast-forward into the verified base branch, source branch retained.
...
Resuming an interrupted burn: re-derive which plan was in flight from the branch name and git state rather than from the transcript, then hand back to `execute-plan`, which owns re-verifying the last task's actual state before anything re-runs.

## Queue end

When the last plan lands, offer the closeout and execute it only on approval: `trash` the `PLAN-*` files (never `rm`), route branch pruning to `git-hygiene`, and save a handoff through `handoff:save-handoff`.
