...
from scripts.ticket_engine_runner import dispatch_stage, load_runner_context, run
...
def test_load_runner_context_preserves_existing_origin_mismatch_behavior(
    tmp_tickets: Path,
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    assert error.state == "escalate"
    assert error.error_code == "origin_mismatch"
