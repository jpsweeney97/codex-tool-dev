...
class TestValidateFields:
    ...
    def test_valid_acceptance_criteria(self):
        errors = validate_fields({"acceptance_criteria": ["criterion one", "criterion two"]})
        assert not errors

    def test_acceptance_criteria_scalar_rejected(self):
        errors = validate_fields({"acceptance_criteria": "criterion one"})
        assert any("acceptance_criteria" in e for e in errors)

    def test_acceptance_criteria_non_string_elements_rejected(self):
        errors = validate_fields({"acceptance_criteria": ["criterion one", 42]})
        assert any("acceptance_criteria" in e for e in errors)
