...
def _normalize_acceptance_criteria(
    ...
    """Validate acceptance_criteria before rendering checklist items."""
    if acceptance_criteria is None:
        return None
    if not isinstance(acceptance_criteria, list):
        raise ValueError(
            "render_ticket failed: acceptance_criteria must be a list of strings. "
            f"Got: {acceptance_criteria!r:.100}"
        )
    if not all(isinstance(item, str) for item in acceptance_criteria):
        raise ValueError(
            "render_ticket failed: acceptance_criteria must contain only strings. "
            f"Got: {acceptance_criteria!r:.100}"
        )
