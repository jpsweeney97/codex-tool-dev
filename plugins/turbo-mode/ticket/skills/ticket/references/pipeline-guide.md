...
The primary `/ticket` UX uses `ticket_workflow.py prepare` and
`ticket_workflow.py execute`. The lower-level classify → plan → preflight →
execute commands remain documented here for debugging, tests, and emergency
operator recovery.

Command examples use `<PROJECT_ROOT>/.codex/ticket-tmp/payload.json` as a placeholder. Substitute your unique absolute `PAYLOAD_PATH` from Setup step 4 wherever this appears and reuse that same path for all four stages of one operation. Relative payload paths are not supported by the guard hook.
...
```json
{
  ...
  "fields": {
    ...
    "acceptance_criteria": [
      "Expired tokens trigger the expected retry path",
      "Concurrent requests no longer fail under load"
    ],
...
For `create`, `acceptance_criteria` must be `list[string]`. Bare strings are invalid input and are rejected before rendering.
...
4. Re-run from `plan` (not from `classify` — `intent` is already in the payload):
   ```bash
   python3 -B <PLUGIN_ROOT>/scripts/ticket_engine_user.py plan <PAYLOAD_PATH>
   python3 -B <PLUGIN_ROOT>/scripts/ticket_engine_user.py preflight <PAYLOAD_PATH>
   python3 -B <PLUGIN_ROOT>/scripts/ticket_engine_user.py execute <PAYLOAD_PATH>
   ```
...
2. Read the existing ticket: `python3 -B <PLUGIN_ROOT>/scripts/ticket_read.py query <TICKETS_DIR> <duplicate_of>`
