...
class TestEngineExecute:
    ...
    def test_execute_create_rejects_scalar_acceptance_criteria(self, tmp_tickets):
        """Create rejects scalar acceptance_criteria before rendering."""
        resp = engine_execute(
            action="create",
            ticket_id=None,
            fields={
            ...
            classify_intent="create",
            classify_confidence=0.95,
            dedup_fingerprint=compute_dedup_fp("Scalar acceptance_criteria must be rejected.", []),
        )
        assert resp.state == "need_fields"
        assert resp.error_code == "need_fields"
        assert "acceptance_criteria" in resp.message
