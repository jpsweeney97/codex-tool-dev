...
Use a literal absolute value such as `PLUGIN_ROOT="/absolute/path/to/handoff/1.7.0"`.
For example, if the skill file is `/absolute/path/to/handoff/1.7.0/skills/distill/SKILL.md`, then `PLUGIN_ROOT` is `/absolute/path/to/handoff/1.7.0`.
...
UV_PROJECT_ENVIRONMENT="$PROJECT_ROOT/.codex/plugin-runtimes/handoff-1.7.0" \
...
| Script path does not resolve | Re-resolve `PLUGIN_ROOT` as the plugin root three levels above this `SKILL.md` |
