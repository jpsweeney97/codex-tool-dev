...
```bash
codex plugin install ./plugins/turbo-mode/handoff/1.7.0
```
...
Handoff `1.7.0` does not ship plugin-bundled command hooks. The dormant hook-compatible scripts are retained in source, but the installed plugin manifest exposes no bundled hook command contract. Plugin-bundled command hooks are deferred from `1.7.0` until Codex provides a documented portable launcher contract or a separate generated-config architecture is approved.
...
```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/handoff/1.7.0 pytest --collect-only -q -p no:cacheprovider
```
