...
Handoff source lives under `plugins/turbo-mode/handoff/1.7.0/`.
...
```bash
cd plugins/turbo-mode/handoff/1.7.0
uv sync
```
...
```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/handoff/1.7.0 pytest -q -p no:cacheprovider
```

For release metadata and docs-only changes:

```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/handoff/1.7.0 pytest tests/test_release_metadata.py tests/test_skill_docs.py -q -p no:cacheprovider
```
...
```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/handoff/1.7.0 python -c "import sys; from turbo_mode_handoff_runtime.storage_authority_inventory import main; sys.argv=['storage_authority_inventory','--write']; raise SystemExit(main())"
```
...
The `1.7.0/` directory name is the **frozen marketplace install path** for this
...
`1.7.0` version and the literal `plugins/turbo-mode/handoff/1.7.0` path strings,
and the `plugin.json` `websiteURL`/`privacyPolicyURL`/`termsOfServiceURL` fields
intentionally embed the frozen `1.7.0` path. A version bump updates only the
