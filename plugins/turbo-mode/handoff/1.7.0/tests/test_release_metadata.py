...
def test_readme_documents_current_summary_and_development_commands() -> None:
    ...
    assert "codex plugin install ./plugins/turbo-mode/handoff/1.7.0" in text
    assert "cd plugins/turbo-mode/handoff/1.7.0" in text
    assert "uv run --directory plugins/turbo-mode/handoff/1.7.0 pytest" in text
...
def test_readme_does_not_publish_bundled_hook_launcher_contract() -> None:
    ...
    assert "plugin-bundled command hooks are deferred from 1.7.0" in text
