...
import pytest
from scripts.installed_host_harness import (
...
def test_source_harness_isolation_proof_uses_test_only_copy_outside_source(
    ...
    helper_path = Path(proof["resolved_helper_path"])
    skill_doc_path = Path(proof["resolved_skill_doc_path"])
    module_paths = [Path(path) for path in proof["loaded_handoff_module_files"]]
    ...
    assert not helper_path.is_relative_to(SOURCE_CHECKOUT_ROOT.resolve())
    assert not skill_doc_path.is_relative_to(SOURCE_CHECKOUT_ROOT.resolve())
    assert all(path.is_relative_to(installed_root) for path in module_paths)
    assert all(not path.is_relative_to(SOURCE_CHECKOUT_ROOT.resolve()) for path in module_paths)
...
def test_source_harness_payload_rejects_source_checkout_helper_leakage(
    ...
    payload = {
        ...
        "resolved_helper_path": str(source_plugin / "scripts" / "session_state.py"),
        "resolved_skill_doc_path": str(source_plugin / "skills" / "save" / "SKILL.md"),
        "helper_subprocess_command_paths": [str(source_plugin / "scripts" / "session_state.py")],
        ...
        "loaded_handoff_module_files": [str(source_plugin / "scripts" / "session_state.py")],
        "manifest_identity": {
            "name": "handoff",
            "version": "1.6.0",
            "source_sha256": "0" * 64,
