...
- When the outcome arrives as a recommendation close (`making-recommendations` or its like: a pick with its case and its flip conditions), the pick is the chosen approach and not the design: take it as the answer to the approaches step, treat the premises its case marked as assumptions as open until your own grounding confirms them, and run the rest of the workflow to an approval. The case for the pick was not an approval of a design, so a plan written straight from it is a plan whose design nobody approved; if the user wants the plan now, say that and ask.
