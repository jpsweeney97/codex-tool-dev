...
- `deliberate` — run Generate, Prune, Shape, Recommend, and Contest on one decision without stopping to ask, and return the recommendation, a ledger of every cut with its revival condition, and a check of the cuts against the recommendation. Each stage uses a fresh agent when supported; otherwise the stages run in the current context and the close discloses that they were not isolated.
...
| `decision-record` | `docs/adr/NNNN-slug.md` at the next number, created lazily; on a supersession, a `Status` line on the older record; on a narrowing that leaves the older record in force, a cross-reference in the new record's prose plus, where the repo's ADR convention carries dated amendment sections, one such section on the older record where that convention places it, its existing text unedited; then a local commit of only those files (`docs(adr): record NNNN <slug>`), skipped on a protected branch or amid unrelated dirty state. A same-decision rerun reuses the existing record and changes it only within the skill’s existing mutation boundary; otherwise it leaves it unchanged. Never pushes, opens a PR, or publishes. |

Companions move with their skills: `outcome-shaping` ships `agents/openai.yaml` and `examples/interaction-examples.md`; `option-shaping` ships `agents/openai.yaml`; `making-recommendations` ships `agents/openai.yaml`, `examples/behavior-examples.md`, and `references/high-stakes.md`; `deliberate` ships `agents/openai.yaml` only: its Prune and Contest methods are inline in its `SKILL.md`, and it runs its sibling stages by having each stage read the sibling's `SKILL.md`. The plugin also ships `references/ADR-FORMAT.md`, the ADR format `decision-record` follows; in the source repo `grill-with-docs` and `improve-codebase-architecture` read that same file through a symlink alias at `skills/grill-with-docs/ADR-FORMAT.md`, so a change to it is a release of this plugin. No hooks are registered and nothing runs unattended.
