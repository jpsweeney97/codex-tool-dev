...
| `decision-record` | `docs/adr/NNNN-slug.md` at the next number, created lazily; on a supersession, a `Status` line on the older record; then a local commit of only those files (`docs(adr): record NNNN <slug>`), skipped on a protected branch or amid unrelated dirty state. Never pushes, opens a PR, or publishes. |
