...
Explicit review-family invocation (including namespaced forms such as `review-family:system-design-review`) wins. This skill wins — including over `scrutinize` — when the requested lens is architecture or system design rather than general adversarial critique; the target shapes it owns are listed in the description:
...
If no material findings surface, say so directly. End with 2-4 sharp questions, not a verdict. If a verdict is requested, route readiness or pass/fail to `scrutinize`; answer design soundness here in the decision states. Save to `docs/audits/YYYY-MM-DD-<target-slug>.md` only when explicitly asked; if the workspace already uses `docs/audits/`, suggest that path instead of writing it.
