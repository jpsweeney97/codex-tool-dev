...
def gate_for(text: str, req_id: str) -> tuple[str, str, str, str, str]:
    ...
    if req_id.startswith("IH-") or "installed-host smoke" in lower:
        return ("gate-5-installed-certification", "installed-harness", "installed-host behavior proof; excluded from source repaired", "Gate 5 budget applies if certification is claimed", "not-claimed")
    if "source-harness" in lower or "gate 1f" in lower:
        return ("gate-1f-installed-host-harness-slice", "installed-harness", "source-harness isolation pytest selector", "Gate 1f split trigger from capacity model", "pending")
    if any(term in lower for term in ("active-writer", "save/summary/quicksave", "begin-active-write", "write-active-handoff", "allocate-active-path")):
        return ("gate-1d-active-writer-operation-state-slice", "active-writer", "active-writer operation-state tests", "Gate 1d split trigger from capacity model", "pending")
    if any(term in lower for term in ("legacy state", "state bridge", "chain-state", "resume token", "state-like residue")):
        return ("gate-1e-state-bridge-and-recovery-slice", "state-recovery", "state bridge and recovery tests", "Gate 1e split trigger from capacity model", "pending")
    if any(term in lower for term in ("load", "transaction", "registry", "archive copy", "copied legacy")):
        return ("gate-1c-load-transaction-slice", "load-transaction", "load transaction pytest selector", "Gate 1c split trigger from capacity model", "pending")
    if any(term in lower for term in ("list", "search", "triage", "distill", "history", "previous-primary", "no-frontmatter")):
        return ("gate-1b-reader-history-slice", "reader-history", "reader/history pytest selectors", "Gate 1b split trigger from capacity model", "pending")
    if any(term in lower for term in ("skill docs", "launcher", "readme", "changelog")):
        return ("gate-2-skill-docs-release-docs", "skill-docs", "skill-doc and release-doc tests", "Gate 2 split trigger from capacity model", "pending")
    if any(term in lower for term in ("refresh", "storage_authority_inventory", "stale-text", "gate_proof_map")):
        return ("gate-3-refresh-and-stale-text", "refresh", "refresh and stale-text checks", "Gate 3 split trigger from capacity model", "pending")
    if req_id.startswith("HS-"):
        return ("gate-4-source-closeout", "closeout", "hard-stop closeout matrix evidence", "Gate 4 split trigger from capacity model", "pending")
    return ("gate-1a-discovery-read-only-slice", "storage-discovery", "storage discovery pytest selector", "Gate 1a split trigger from capacity model", "pending")


def required_tests(plan_text: str) -> list[tuple[str, str, str]]:
    bullets = markdown_bullets(section_body(plan_text, "Required Tests"))
    return [(f"RT-{index:03d}", "Required Tests", bullet) for index, bullet in enumerate(bullets, start=1)]


def installed_host_rows(plan_text: str) -> list[tuple[str, str, str]]:
    table_rows = parse_markdown_table(subsection_body(plan_text, "Installed Host Repo Policy Matrix"))
    rows: list[tuple[str, str, str]] = []
    for index, row in enumerate(table_rows, start=1):
        rows.append((f"IH-{index:03d}", "Installed Host Repo Policy Matrix", normalize_text(f"{row['Host shape']}: {row['Required behavior']}")))
    return rows
...
def compatibility_rows(plan_text: str) -> list[tuple[str, str, str]]:
    ...
    for row in rows:
        ...
        if decision in kept:
            index = len(output) + 1
            output.append((f"CL-{index:03d}", "API And CLI Compatibility Ledger", normalize_text(f"{row['Surface']}: {decision}: {row['Required behavior']}")))
    return output


def hard_stop_rows(matrix_text: str) -> list[tuple[str, str, str]]:
    rows = parse_markdown_table(matrix_text)
    return [(row["stop_id"], "Hard Stop Closeout Matrix", normalize_text(row["stop_condition"])) for row in rows]


def stale_bootstrap_row() -> list[tuple[str, str, str]]:
    return [("ST-BOOTSTRAP", "Generated Stale-Text Gate", "Create storage_authority_inventory.py, the storage authority inventory fixture, and a no-write --check contract in Gate 3; generated stale-text rows are not Gate 0r inputs.")]
...
def build_rows(plan_text: str, matrix_text: str) -> list[dict[str, str]]:
    ...
    sources.extend(stale_bootstrap_row())
...
def check_rows(plan_text: str, matrix_text: str, map_text: str) -> None:
    ...
    if set(expected_by_id) != set(actual_by_id):
        ...
        raise ToolError(f"gate_proof_map check failed: id drift. Got: missing={missing!r} extra={extra!r}")
    for req_id, expected_row in expected_by_id.items():
        ...
        if actual_row.get("first_enforcing_gate") not in BUDGETS:
            raise ToolError(f"gate_proof_map check failed: unknown gate. Got: {actual_row.get('first_enforcing_gate')!r}")
    ...
    for gate, count in counts.items():
        ...
        if over_budget:
            ...
            if not all(row.get("split_trigger") for row in rows):
                raise ToolError(f"gate_proof_map check failed: over budget without split trigger. Got: {gate!r} count={count!r}")
    hard_stop_hashes = {row["stop_id"]: row["stop_condition_sha256"] for row in parse_markdown_table(matrix_text)}
    for req_id, matrix_hash in hard_stop_hashes.items():
        ...
        if actual_hash != matrix_hash:
            raise ToolError(f"gate_proof_map check failed: hard-stop hash mismatch. Got: {req_id!r}")
