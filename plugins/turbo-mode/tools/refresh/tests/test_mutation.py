...
def test_seed_isolated_rehearsal_home_uses_installed_cache_when_plan05_fixture_stale(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    init_git_repo(repo_root)
    live_home = tmp_path / "real-home"
    codex_home = tmp_path / "isolated-home"
    ticket_hook_command = (
        f"python3 {live_home}/plugins/cache/turbo-mode/ticket/1.4.0/"
        "hooks/ticket_engine_guard.py"
    )
    ticket_hook_text = json.dumps(ticket_hooks_payload(ticket_hook_command))
    write_repo_file(
        repo_root,
    ...
    write_repo_file(
        repo_root,
        "plugins/turbo-mode/ticket/1.4.0/hooks/hooks.json",
        ticket_hook_text,
    )
    write_repo_file(
        repo_root,
        "plugins/turbo-mode/ticket/1.4.0/hooks/ticket_engine_guard.py",
        "#!/usr/bin/env python3\nprint('guard')\n",
    )
    source_commit = commit_repo(repo_root, "source")
    source_tree = git(repo_root, "rev-parse", f"{source_commit}^{{tree}}")

    live_handoff = live_home / "plugins/cache/turbo-mode/handoff/1.6.0/README.md"
    live_handoff.parent.mkdir(parents=True)
    live_handoff.write_text("stale handoff cache\n", encoding="utf-8")
    live_ticket_root = live_home / "plugins/cache/turbo-mode/ticket/1.4.0"
    (live_ticket_root / "hooks").mkdir(parents=True)
    (live_ticket_root / "README.md").write_text("ticket source\n", encoding="utf-8")
    (live_ticket_root / "hooks/hooks.json").write_text(ticket_hook_text, encoding="utf-8")
    (live_ticket_root / "hooks/ticket_engine_guard.py").write_text(
        "#!/usr/bin/env python3\nprint('guard')\n",
        encoding="utf-8",
    )

    class FakeTerminalStatus:
        value = "guarded-refresh-required"

    class FakePlan:
        terminal_status = FakeTerminalStatus()

    def fake_plan_refresh(**_kwargs: object) -> FakePlan:
        return FakePlan()

    def fake_write_local_evidence(_result: object, *, run_id: str) -> Path:
        path = codex_home / "local-only/turbo-mode-refresh" / run_id / "dry-run.summary.json"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"terminal_plan_status": "guarded-refresh-required"}),
            encoding="utf-8",
        )
        return path

    monkeypatch.setattr(mutation_module, "REAL_CODEX_HOME", live_home)
    monkeypatch.setattr(mutation_module, "plan_refresh", fake_plan_refresh)
    monkeypatch.setattr(mutation_module, "write_local_evidence", fake_write_local_evidence)

    result = mutation_module.seed_isolated_rehearsal_home(
        repo_root=repo_root,
    ...
    assert result.canonical_drift_paths == ("handoff/1.6.0/README.md",)
    manifest = json.loads(Path(result.seed_manifest_path).read_text(encoding="utf-8"))
    assert manifest["seed_source"] == "installed-cache-readonly"
    assert manifest["canonical_drift_paths"] == ["handoff/1.6.0/README.md"]
    assert (
        codex_home / "plugins/cache/turbo-mode/handoff/1.6.0/README.md"
    ).read_text(encoding="utf-8") == "stale handoff cache\n"
    isolated_hook = json.loads(
        (codex_home / "plugins/cache/turbo-mode/ticket/1.4.0/hooks/hooks.json").read_text(
            encoding="utf-8"
        )
    )
    assert isolated_hook["hooks"]["PreToolUse"][0]["hooks"][0]["command"] == (
...
def test_live_guarded_orchestration_retains_marker_when_smoke_rollback_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from contextlib import contextmanager
    ...
    seed_live_rehearsal_capture(ctx)
    launch = launch_authority(ctx)
    marker_path = ctx.local_only_run_root.parent / "run-state" / f"{ctx.run_id}.marker.json"
    ...
    def failing_rollback(
        ...
        *,
        ...
        assert failed_phase == "smoke"
        raise RefreshError("forced rollback failure")
    ...
    monkeypatch.setattr(mutation_module, "rollback_guarded_refresh", failing_rollback)
    ...
    assert result.final_status == "MUTATION_FAILED_ROLLBACK_FAILED"
    assert marker_path.exists()
    final_status = json.loads(Path(result.final_status_path).read_text(encoding="utf-8"))
    assert final_status["failure_reason"] == (
        "forced smoke failure; rollback failed: forced rollback failure"
    )


def test_live_guarded_orchestration_retains_marker_when_smoke_status_rollback_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from contextlib import contextmanager
    ...
    monkeypatch.setattr(
        mutation_module,
        "run_standard_smoke",
        lambda **_kwargs: {"final_status": "failed"},
    )
    monkeypatch.setattr(mutation_module, "rollback_guarded_refresh", failing_rollback)
    ...
    assert final_status["failure_reason"] == (
        "run standard smoke failed: final status is not passed. Got: 'failed'; "
        "rollback failed: forced rollback failure"
    )
...
def test_live_guarded_orchestration_rolls_back_when_post_mutation_process_gate_blocks(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from contextlib import contextmanager
    ...
    def fake_process_gate(**kwargs: object) -> dict[str, object]:
        ...
        payload = {
            "label": label,
            "blocked_process_count": 1 if label == "post-mutation" else 0,
            "exclusivity_status": "blocked" if label == "post-mutation" else (
                "exclusive_window_observed_by_process_samples"
            ),
        }
        write_json(ctx.local_only_run_root / f"process-{label}.summary.json", payload)
        return payload
    ...
    monkeypatch.setattr(
        mutation_module,
        "run_standard_smoke",
        lambda **_kwargs: {"final_status": "passed"},
    )
    monkeypatch.setattr(mutation_module, "rollback_guarded_refresh", fake_rollback)
    ...
    assert result.final_status == "MUTATION_FAILED_ROLLBACK_COMPLETE"
    assert rollback_calls == ["post-mutation"]
    assert not marker_path.exists()
    final_status = json.loads(Path(result.final_status_path).read_text(encoding="utf-8"))
    assert final_status["failure_reason"] == (
        "run process gate failed: process gate blocked. Got: 'post-mutation'"
    )


def test_live_guarded_orchestration_retains_marker_when_post_mutation_rollback_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from contextlib import contextmanager
    ...
    def failing_rollback(
        ...
        *,
        ...
        assert failed_phase == "post-mutation"
        raise RefreshError("forced rollback failure")
