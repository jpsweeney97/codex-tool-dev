...
7. **Generate markdown** with frontmatter per [format-reference.md](../../references/format-reference.md) and [handoff-contract.md](../../references/handoff-contract.md):
   ...
   - Per chain protocol in [handoff-contract.md](../../references/handoff-contract.md): read `<project_root>/docs/handoffs/.session-state/handoff-<project>` — if exists, set `resumed_from` to its content
...
9. **Cleanup state file** per chain protocol in [handoff-contract.md](../../references/handoff-contract.md):
   - `trash` the state file at `<project_root>/docs/handoffs/.session-state/handoff-<project>` if it exists. If `trash` fails, warn the user that the state file persists but do not block — the 24-hour TTL will clean it up.
