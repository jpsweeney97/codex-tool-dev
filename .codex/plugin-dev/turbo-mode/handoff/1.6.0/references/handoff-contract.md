...
2. Write archive path to `<project_root>/docs/handoffs/.session-state/handoff-<project>`

**Save/Quicksave/Summary (save, quicksave, summary) — reads and cleans state:**
1. **Read:** Check `<project_root>/docs/handoffs/.session-state/handoff-<project>` — if exists, include path as `resumed_from` in frontmatter
2. **Write:** Write the new handoff/checkpoint file
3. **Cleanup:** Use `trash` to remove state file at `<project_root>/docs/handoffs/.session-state/handoff-<project>` (if exists). If `trash` fails, warn the user that the state file persists but do not block handoff/checkpoint creation — the 24-hour TTL will clean it up.
...
| `<project_root>/docs/handoffs/.session-state/handoff-<project>` | Plain text (path) | 24 hours |
...
- `docs/handoffs/` (active and archive) is gitignored alongside other ephemeral state (`docs/decisions/`, `.codex/sessions/`).
