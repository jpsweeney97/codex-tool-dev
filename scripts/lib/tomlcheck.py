...
def lint_automation_templates(templates_dir: Path, only_name: str | None = None) -> list[str]:
    ...
    if not templates_dir.exists():
        return [f"lint_automation_templates failed: missing templates dir. Got: {str(templates_dir)!r}"]

    for tmpl in templates_dir.glob("*.toml.tmpl"):
        if only_name is not None and tmpl.stem.replace(".toml", "") != only_name and tmpl.stem != only_name:
