...
- proof that `plugin/read`, `plugin/list`, `skills/list`, and `hooks/list` responses came from the requested home and marketplace, not from an implicit default home. When pre-install target authority is collected while `features.plugin_hooks = false`, its `hooks/list` response is a hook-disabled runtime proof and must record zero hooks; after config restore, same-child and fresh-child post-install corroboration remain hook-certifying proofs.
...
- pre-install target authority collected while `features.plugin_hooks = false` asserts expected installed plugin identity through `plugin/read`, `plugin/list`, and `skills/list`, and records `hooks/list` as hook-disabled zero-hook evidence;
- same-child and fresh-child post-install app-server corroboration run after config restore and assert expected installed plugin identity through `plugin/read`, `plugin/list`, `skills/list`, and `hooks/list`, including source marketplace path and installed skill/hook paths for both Handoff and Ticket;
...
28. Run `plugin/install` for Handoff and Ticket only after the marker contains that pre-install target authority digest. Assert sparse success/auth schema and request-id correlation for both install responses, rewrite the installed Ticket hook manifest to the active Codex home, restore config bytes on disk, then collect hook-certifying same-child post-install app-server corroboration with `plugin/read`, `plugin/list`, `skills/list`, and `hooks/list`.
...
REHEARSAL_PROOF_PATH="$ISOLATED_CODEX_HOME/local-only/turbo-mode-refresh/$RUN_ID/rehearsal-proof.json"
...
- app-server authority proof records no config, cache, plugin, hook, local-only, or installed path under `/Users/jp/.codex`; pre-install authority may record a hook-disabled zero-hook response, while same-child and fresh-child post-install evidence after config restore must record the isolated Ticket hook path;
