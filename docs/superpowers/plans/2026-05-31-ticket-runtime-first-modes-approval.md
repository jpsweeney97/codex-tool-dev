...
**Architecture:** This is the first source implementation slice from the Ticket source-runtime drift ledger. It changes the local mode model, runtime evaluator, pending-summary validation, gateway validation, and apply-turn projection together so one producer is not removed while consumers still require it. Diagnostic dry-run remains a future explicit maintenance affordance; it is not represented as a durable mode, runtime decision kind, or product state in this slice.
...
```bash
rg -n "AutomationMode\\.PREVIEW|RuntimeDecisionKind\\.PREVIEW_ONLY|preview_only|\\\"preview\\\"|approval_consumed|decision\\.approval|details\\[\\\"approval\\\"\\]" plugins/turbo-mode/ticket/tests/test_autonomy_config.py plugins/turbo-mode/ticket/tests/test_autonomy_runtime.py plugins/turbo-mode/ticket/tests/test_turn_batch.py plugins/turbo-mode/ticket/tests/test_engine_gateway.py plugins/turbo-mode/ticket/tests/test_autonomy_cli.py
```
...
- [ ] **Step 5: Update preview setup-choice CLI expectation**
...
- [ ] **Step 6: Run focused runtime and CLI tests**
...
- [ ] **Step 7: Commit runtime preview removal**
...
In `plugins/turbo-mode/ticket/scripts/ticket_engine_gateway.py`, delete `_approval_ticket_id()` and `_parse_z()` if no other code uses them.
...
```python
    decision_error = _decision_error(mutation=mutation, decision=decision)
...
## Task 6: Update Apply-Turn Recovery Tests And Source Searches
...
```bash
rg -n "approval_consumed|details\\[\\\"approval\\\"\\]|decision\\.approval|approval_id" plugins/turbo-mode/ticket/tests/test_autonomy_cli.py plugins/turbo-mode/ticket/tests/test_engine_gateway.py plugins/turbo-mode/ticket/tests/test_turn_batch.py plugins/turbo-mode/ticket/tests/test_autonomy_runtime.py
```

Expected after Tasks 3-5: remaining matches should be either legacy recovery fixtures or tests that still need source-slice rewrite. New successful write paths must not expect `approval_consumed`.
...
Apply the same helper rewrite in `plugins/turbo-mode/ticket/tests/test_engine_gateway.py` if it was not already handled in Task 4.

- [ ] **Step 3: Keep legacy `approval_consumed` fixtures marked as legacy**
...
- [ ] **Step 4: Confirm no new source writes approval envelopes or preview decisions**
...
- [ ] **Step 5: Run focused apply-turn and gateway suites**

Run:

```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/ticket pytest tests/test_autonomy_cli.py tests/test_engine_gateway.py tests/test_turn_batch.py tests/test_autonomy_runtime.py tests/test_autonomy_config.py -q
```

Expected: PASS.

- [ ] **Step 6: Commit apply-turn test cleanup**

Run:

```bash
git add plugins/turbo-mode/ticket/tests/test_autonomy_cli.py plugins/turbo-mode/ticket/tests/test_engine_gateway.py
...
```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/private/tmp/codex-tool-dev-pycache uv run --directory plugins/turbo-mode/ticket pytest tests/test_autonomy_config.py tests/test_autonomy_runtime.py tests/test_turn_batch.py tests/test_engine_gateway.py tests/test_autonomy_cli.py -q
```
...
- [ ] **Step 7: Record closeout commit if final cleanup changed files**
...
- Gateway validation uses `AutonomyDecision`, `CandidateMutation`, and `GatewayMutation` fields already present in current source.
