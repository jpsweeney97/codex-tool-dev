...
- `target` has exactly two keys: `fields` and `sections`.
...
- `proposed_change` keys must exactly equal the union of `target.fields` and
  `target.sections`. Every named target field or section must appear in
  `proposed_change`, and every `proposed_change` key must be named in
  `target`.
- `proposed_change` contains only values for the named target fields or
  ...
  mutation IDs, evidence objects, raw operation-log data, or nested control
  fields.
...
- Ticket computes candidate identity from canonical candidate content and the
  live target fingerprint for non-create writes. For `create`, candidate
  identity is computed from the canonical pre-allocation candidate content.
  Callers do not supply authoritative identity values.
...
- `create`: `ticket_id` and `expected_ticket_fingerprint` are `null`.
  ...
  currently actionable. `create` with `status: done` or `status: wontfix` is
  invalid in normal mutation flow. Ticket allocates `id`, validates the
  status-specific shape, and appends the first `Change History` entry.
...
Trust guarantee: retrying an interrupted `create` does not create a duplicate
ticket while the relevant operation-log binding is retained.

Implementation rule: for `create`, candidate identity is computed from the
canonical create candidate before kernel-owned allocation. It includes the
action, target, proposed user-visible content, and `evidence_summary`. It
excludes allocated `id`, generated `Change History`, timestamps,
operation-log data, and other kernel-owned side effects. Under the write lock,
Ticket records a retained binding from that candidate identity to the allocated
ticket ID before writing the ticket file. A retry of the same create candidate
uses that binding while retained.

Create idempotency is limited to retained operation-log history. After the
binding expires, Ticket evaluates a similar create as a fresh request against
current ticket state, normal duplicate/current-state checks, caps, and
Codex/user judgment. Ticket does not infer exact duplicate identity from similar
ticket prose or matching target content after the retained allocation binding
expires.

### Exceptions Outside Normal Candidate Mutation

Migration/import paths and diagnostic repair paths are explicit exceptions
outside the normal candidate mutation contract.

Migration/import paths convert or reconstruct ticket records from an accepted
external or legacy source. They may create terminal tickets only when preserving
already-terminal source records.

Diagnostic repair paths fix malformed target files, corrupted `Change History`,
or other broken Ticket storage. They may rewrite existing `Change History` only
when the operator invokes a repair command that names the repair purpose and
reports the changed history entries.

Neither exception runs as part of ordinary `create`, `update`, `done`,
`wontfix`, `reopen`, `correct`, or `reconcile_board`.
...
`Blocked On` is the required visible blocker condition for `status: blocked`.
`blocked_by` is optional ticket-ID dependency data and contains only ticket IDs.
It is invalid as a live blocker field on non-blocked tickets. Ticket validates
`blocked_by` shape and may use resolved ticket-ID dependencies as mechanical
evidence that an unblock candidate is worth proposing. `blocked_by` does not
replace `Blocked On`, and Ticket does not decide prose blocker truth by itself.
Codex owns the semantic judgment that a `Blocked On` prose condition is no
longer true.

Unblocking remains an ordinary `blocked -> open` candidate. It names `status`,
`blocked_by`, `Blocked On`, and `Next Action`; the same write clears
`blocked_by`, removes `Blocked On`, rewrites `Next Action` to the continuation
step, and appends a `Change History` reason. Historical blocker context belongs
only in `Change History`.
...
choices that Ticket cannot infer mechanically. Reconciliation candidates above
the reconciliation cap are wrapper-level `overflow`, not candidate-level
`needs_discussion`, when the bounded subset is applied successfully.
`needs_discussion` is not a Codex confidence state, uncertainty score, semantic
hesitation label, or substitute for judging whether the human-readable reason is
strong enough. If Codex can make a bounded allowed write and state the reason
honestly, it should emit a valid candidate; if it cannot formulate the target or
reason honestly, Codex should discuss in ordinary chat instead of encoding
uncertainty in Ticket state.
...
Trust guarantee: Ticket recovery does not duplicate writes, lose completed
writes, or hide completed writes from the user.

Implementation rule: the operation log records retained recovery facts, not
copied ticket content and not a workflow-stage enum. Recovery branches on the
presence, absence, and consistency of those facts.

The log may retain only these durable private facts:

- candidate identity;
- action, limited to the six target actions;
- target ticket ID, including the allocated ticket ID for `create`;
- target fields or sections;
- expected pre-write recovery fact;
- expected post-write fingerprint;
...
- exact generated `Change History` metadata for every appended history entry;
- short mechanical failure or pause reason;
- timestamp;
- bounded correction detail for recent corrections.

For non-create writes, the expected pre-write recovery fact is the expected
target ticket fingerprint. For `create`, the expected pre-write recovery fact is
that the allocated target ID path is unused; there is no pre-existing ticket
fingerprint.

The expected post-write fingerprint covers the full canonical target ticket
after the candidate write and all required kernel-owned side effects, including
the generated `Change History` append. When Ticket generates a `Change History`
entry, the log retains the exact bounded kernel metadata needed to reproduce
that entry byte-for-byte, including the timestamp and rendered reason text or
equivalent exact rendered components. This applies to every candidate write
that generates a history append, including `correct`.

Exact rendered `Change History` metadata is not copied ticket content or a
shadow ticket store; it is the audit line for the candidate write.
...
When ticket write completion is not recorded, recovery compares current state to
the retained pre-write and post-write facts:

- If current state matches the expected pre-write fact, Ticket retries the
  write.
- If current state matches the expected post-write fingerprint, Ticket records
  write completion and proceeds to summary recovery without rewriting.
- If current state matches neither, automatic mutation pauses with
  `invalid_state` or `blocked`.

For `create`, recovery retries the write at the allocated ID only after
verifying the allocated target ID path is still unused. If that path is occupied
by a ticket matching the expected post-write fingerprint, Ticket records write
completion and proceeds to summary recovery without creating another ticket. If
the path is occupied by anything else, automatic mutation pauses with
`invalid_state` or `blocked`.

When ticket write completion is recorded, Ticket does not rewrite the ticket
from recovery. If the current target still matches the expected post-write
fingerprint and summary emission was not recorded, Ticket reports the
already-applied mutation and records summary emission. If both write completion
and summary emission are recorded, retry returns already-applied or `no_change`
behavior without duplicating either the write or the summary. If the current
target is missing or no longer matches the expected post-write fingerprint,
automatic mutation pauses rather than recreating completed history.

Operation-log retention bounds exact recovery guarantees. When the relevant
retained facts have expired, Ticket must not claim that an old candidate is
already applied or exactly duplicate based on missing log evidence. The
candidate is evaluated as a fresh request against current ticket state, normal
duplicate/current-state checks, caps, and Codex/user judgment.
...
safely update within the reconciliation caps and create missing tickets for
relevant follow-ups it can honestly describe within those caps. New follow-ups
may be created directly as `open` when they are actionable now or as `idea` when
they are real enough to remember but not yet actionable. Low importance,
speculative value, or "maybe later" uncertainty is not by itself a reason to
drop a follow-up from discovery; Codex expresses that in visible ticket content,
priority, tags, or `idea` status. Duplicates and malformed or unsupported ticket
content remain blockers.
...
reconciliation write. It counts against reconciliation caps and still obeys
ordinary non-create write safety rules, including expected fingerprint, valid
target, and deterministic transition checks.

Trust guarantee: reconciliation may search broadly, but it does not silently
turn a claim point into an unbounded write batch.

Implementation rule: reconciliation has a default cap of 15 applied candidates
per wrapper call. `done` and `wontfix` are terminal lifecycle writes and have a
separate cap of five applied candidates within that 15-candidate total unless
the user explicitly requested a linked terminal batch. `correct`, `update`,
`reopen`, and `create` do not consume the terminal lifecycle cap, even when they
touch a terminal ticket. Normal `create` cannot create `done` or `wontfix`
tickets.

Reconciliation applies safe candidates in this order:

1. Explicitly referenced existing tickets from the current user request.
2. Existing tickets directly named or mutated by the current Ticket operation.
3. Existing tickets tied to files changed in the current turn or branch.
4. Existing active `open` or `blocked` tickets whose problem overlaps with what
   Codex learned.
5. Existing `idea` tickets plausibly promotable to `open`.
6. New follow-up creates, in stable order from Codex's generated candidate list.

Within buckets 1-5, terminal writes outrank non-terminal writes, then candidates
are ordered by ticket ID. New follow-up creates are always last.

If terminal candidates exceed the terminal cap, reconciliation applies the
mechanically first terminal candidates up to the terminal cap, places the
remaining terminal candidates in `overflow`, and continues applying
non-terminal candidates until the 15-candidate total cap is reached. Terminal
overflow does not stop otherwise safe non-terminal reconciliation.

When reconciliation exceeds the automatic cap, the wrapper applies the
mechanically first safe subset and returns `ok` with an `overflow` list. The
wrapper summary lists both applied candidates and overflow candidates. Overflow
candidates are compact summaries, not raw candidate payloads: each item names
the action, ticket ID or `null` for create, target fields and sections, and the
one-line reason. Overflow items do not include proposed replacement values.
...
relevant tickets that fit the cap and reports the blocked item in the final
message or handoff. Incomplete ticket reconciliation qualifies claims about
board freshness, but it does not automatically invalidate claims about the
underlying work. Ticket must not persist reconciliation blockers or overflow as
a separate workflow state.
...
- terminal lifecycle writes outside reconciliation are capped at one per turn
  unless the user explicitly requested a linked batch;
- for cap purposes, terminal lifecycle writes are only `done` and `wontfix`
  candidates;
- `reconcile_board` at handoff, status, completion, or other claim points uses
  its own cap of 15 applied candidates, with a separate cap of five terminal
  lifecycle writes inside that 15-candidate total;
- ordinary candidates above the applicable non-reconciliation cap return
  `needs_discussion`;
- reconciliation candidates above the applicable reconciliation cap are not
  applied and appear in wrapper-level `overflow` while the wrapper may still
  return `ok`;
...
candidate selection, duplicate judgment, and the stable generated order for new
follow-up creates.
...
- Verification proves migration/import and diagnostic repair exceptions do not
  run through ordinary `create`, `update`, `done`, `wontfix`, `reopen`,
  `correct`, or `reconcile_board` paths.
